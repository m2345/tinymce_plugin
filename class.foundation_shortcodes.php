...
class WP_Foundation_shortcodes_shortcodes{
	function __construct(){
		...
		$i = 0;
		$custom_class_val = ($custom_class) ? $custom_class : '' ;
		...
		$output = '<table class="wpfs '.$custom_class_val.'">';
	...
	public static function alert_box( $atts, $content = null){
		...
		if ($icon == 'yes'):
			switch($color){
                ...
                $output = <<<HTML
                <div class="wpfs callout type-{$type_val} {$domain} {$size} {$color} {$corners} {$custom_class}">{$font_icon}{$alertContent}{$close_btn}</div>
HTML;
                return $output;
	}
	...
	public static function tabs( $atts, $content = null){
		...
		$output .= '<ul class="tabs '.$direction.'" data-tab>';
	...
	public static function tab( $atts, $content = null){
		...
		if ($global_wp_foundation_shortcodes['tab_title']){
			$output = <<<HTML
			<li class="tab-title {$active_class}"><a href="#tabpanel-{$global_wp_foundation_shortcodes['tab_id']}-{$global_wp_foundation_shortcodes['tab_counter']}">{$title}</a></li>
HTML;
		} else {
			$output = <<<HTML
...
HTML;
	...
	// accordions   
		...
		$global_wp_foundation_shortcodes['accordion_id'] = rand();
                $output = '<ul class="wpfs accordion '.$custom_class.'" data-accordion>';
                $output .=  do_shortcode($content);
                $output .= '</ul><!-- /.accordion -->';
		...
		$content = trim($content);
                ...
                $output = <<<HTML
                        <li class="wpfs accordion-navigation">
...
HTML;
	...
	public static function progressbar( $atts, $content = null){
		...
		$value = floatval($value);
                ...
                $output = <<<HTML
                <div class="wpfs progress {$domain} {$corners} {$custom_class}">
...
HTML;
	...
	public static function range_slider( $atts, $content = null){
		...
		$step_data_options = ($step) ? 'step:'.$step.';' : '' ;
                ...
                $output = <<<HTML
                <div class="wpfs range-slider {$direction_class} {$accessibility} {$corners} {$custom_class}" data-slider data-options="{$direction_data_options}{$initial_value_data_options}{$start_value_data_options}{$end_value_data_options}{$step_data_options}">
...
HTML;
	...
	public static function pricing_table( $atts, $content = null){
		...
		$recommended_title_val = ($recommended == 'yes') ? '<li class="best"><strong>'.__('Recommended', 'wp-foundation-shortcodes').'</strong></li>' : '' ;
                ...
                $output = <<<HTML
                <ul class="wpfs pricing-table {$custom_class} {$recommended_class_val}">
...
HTML;
	...
	// button
	//edited by megan 5-9-16
	/*
	 * TODO determine if you should use <button> or <a> tag???
	 * */
	public static function button( $atts, $content = null){
		...
		$span = '<span class="show-for-sr">' . $screen_readers. '</span>';
		$tag_open = ($url) ? '<a href="'.$url.'" target="'.$target.'"' : '<button' ;
		$tag_close = ($url) ? '</a>' : '</button>' ;
		$full_content = $font_icon . $buttonContent;
		if($aria_hidden)
			$full_content = $span . '<span aria-hidden="true">' . $full_content . '</span>';
	
		$output = <<<HTML
			{$tag_open} class="wpfs button {$color} {$dropdown} {$hollow} {$corners} {$size} {$custom_class}"> {$full_content} {$tag_close}
HTML;
                return $output;
	}
	...
	public static function label( $atts, $content = null){
		...
		$labelContent = ($content) ? do_shortcode($content) : __('Label', 'wp-foundation-shortcodes');
                ...
                $output = <<<HTML
                        <span class="wpfs label {$color} {$corners} {$custom_class}">{$font_icon}{$labelContent}</span>
HTML;
	...
	public static function equalizers( $atts, $content = null){
		...
		$output = '<div data-equalizer class="wpfs row '.$custom_class.'">';
		$output .=  do_shortcode($content);
		$output .= '</div><!-- /.data-equalizer -->';
	
		$cnt = 12 / $global_wp_foundation_shortcodes['equalizer_counter'];
		$output = str_replace('@CNT@', $cnt, $output);
	...
	public static function equalizer( $atts, $content = null){
		...
		$output = <<<HTML
		<div class="medium-@CNT@ columns">
		    <div data-equalizer-watch class="panel panel-{$global_wp_foundation_shortcodes['equalizer_counter']} {$custom_class}">
		      <h3>{$title}</h3>
			{$equalizerContent}
		    </div>
		</div>
HTML;
	...
	public static function dropdown( $atts, $content = null){
		...
		$output = <<<HTML
		<div class="wpfs dropdown-wrapper {$custom_class}">
...
HTML;
	...
	// split buttons
		...
		$custom_class = esc_attr($custom_class);
                ...
                $output = <<<HTML
		<div class="wpfs split-button-wrapper {$custom_class}">
...
HTML;
	...
	public static function blockquote( $atts, $content = null){
		...
		$output = <<<HTML
		<blockquote class="wpfs {$custom_class}">{$blockquoteContent}<cite>{$author}</cite></blockquote>
HTML;
	...
	// icon
		...
		$icon_size = ($size) ? 'font-size:'.$size.';' : '' ;
                ...
                $output = <<<HTML
		<i class="wpfs {$custom_class} fa {$icon}" style="{$icon_color}{$icon_size}">{$iconContent}</i>
HTML;
	...
	// address
		...
		$custom_class = esc_attr($custom_class);
                $output = <<<HTML
		<address class="wpfs {$custom_class}">
...
HTML;
	...
	public static function row( $atts, $content = null){
		...
		extract(shortcode_atts(
                ...
                $output = <<<HTML
                <div class="row {$class}">{$rowContent}</div>
HTML;
	...
	public static function columns( $atts, $content = null){
		...
		$output = <<<HTML
                <div class="columns {$class}">{$columnsContent}</div>
HTML;
		return $output;
	}
	...
	public static function blocks_grid( $atts, $content = null){
		$content = trim($content);
                ...
                $output = <<<HTML
                <ul class="wpfs {$class}">{$blocksGridContent}</ul>
HTML;
                return $output;
        }
...
HTML;
	...
	public static function google_map( $atts, $content = null){
		...
		$output = <<<HTML
			<div class="wpfs google-map {$custom_class}">
...
HTML;
	...
	// orbit sliders
		...
		$content = trim($content);
                ...
                $output = <<<HTML
			<ul data-orbit class="wpfs">{$orbitSlidersContent}</ul>
HTML;
                return $output;
        }
		...
		$caption = '<div class="orbit-caption">'.$title.'</div>';
                $output = <<<HTML
...
HTML;
	...
	public static function thumbnail( $atts, $content = null){
		...
		extract(shortcode_atts(
                ...
                $output = <<<HTML
		<a class="wpfs th {$corners} {$custom_class}" role="button" aria-label="Thumbnail" href="{$url}" target="{$target}">
...
HTML;
	...
	public static function clearing_thumbs( $atts, $content = null){
		...
		extract(shortcode_atts(
                ...
                $output = <<<HTML
		<ul class="wpfs clearing-thumbs {$class}" data-clearing>
...
HTML;
	...
	public static function clearing_thumb( $atts, $content = null){
		...
		$data_caption = ($title) ? 'data-caption="'.$title.'"' : '' ;
                $output = <<<HTML
			<li><a class="th" {$href}><img {$data_caption} src="{$clearingThumbContent}"></a></li>
HTML;
                return $output;
        }
	...
	// slick slider
		...
		$output = <<<HTML
			<div class="wpfs slick-slider slider slider-{$id} {$custom_class}">
				{$slickSlidersContent}
			</div>
			<script>
			jQuery(function(){
...
HTML;
	...
	public static function slick_slider( $atts, $content = null){
		...
		$output = <<<HTML
		<div><img src="{$slickSliderContent}" alt=""></div>
HTML;
		return $output;
        }

// button groups
/* TODO individual button options??
 * Does this do anything with those in the original? 
 * */
		...
		if ($hollow) 	$content = str_replace ('[button_group', '[button_group hollow="'.$hollow.'"', $content);
		if ($dropdown) 	$content = str_replace ('[button_group', '[button_group dropdown="'.$dropdown.'"', $content);
		
		$buttonGroupsContent = ($content) ? do_shortcode($content) : '';
                $output = <<<HTML
			<div class="wpfs button-group {$corners} {$size} {$stacked} {$custom_class}">
				{$buttonGroupsContent}
			</div>
HTML;
                return $output;
		...
		extract(shortcode_atts(
                ...
                $output = <<<HTML
			<a href="{$url}" class="button {$color} {$hollow} {$dropdown}">{$buttonGroupContent}</a>
HTML;
                return $output;
        }
	...
	public static function switch_func( $atts, $content = null){
		...
		$label_off_status_val = ($label_off_status) ? '<span class="switch-on-off switch-off">'.$label_off_status.'</span>' : '' ;
                ...
                $output = <<<HTML
		<div class="wpfs switch {$domain} {$corners} {$size} {$custom_class}">
			<input id="{$id}" type="{$type}" name="{$name_val}">
			<label for="{$id}">
...
HTML;
	...
	public static function inline_list( $atts, $content = null){
		...
		$output = <<<HTML
		<ul class="wpfs inline-list {$custom_class}">{$inlineListContent}</ul>
HTML;
		return $output;
	}
	...
	public static function link_func( $atts, $content = null){
		...
		$output = <<<HTML
		<li><a href="{$url}">{$linkContent}</a></li>
HTML;
		return $output;
	}
	...
	public static function keystroke( $atts, $content = null){
                ...
                $output = <<<HTML
		<kbd class="wpfs {$custom_class}">{$keystrokeContent}</kbd>
HTML;
                return $output;
        }
	...
	// panel
		...
		$callout_class = ($callout == 'yes') ? 'callout' : '' ;
                $output = <<<HTML
		<div class="wpfs panel {$corners} {$callout_class} {$custom_class}">{$panelContent}</div>
HTML;
                return $output;
        }
	...
	// tooltip
		...
		switch ($direction){
			...
			break;
                ...
                $output = <<<HTML
		<span data-tooltip aria-haspopup="true" class="wpfs has-tip {$direction_class} {$corners}" title="{$title}">{$tooltipContent}</span>
HTML;
                return $output;
        }
	...
	public static function product_card( $atts, $content = null){
		...
		$output = <<<HTML
		<div class="wpfs product-card-wrapper {$custom_class}">
...
HTML;
	...
	public static function product_card_hover( $atts, $content = null){
		...
		$output = <<<HTML
		<div class="wpfs product-card-hover-wrapper {$hover_effect} {$custom_class}">
	      		<img src="{$img}" alt="">
...
HTML;
	...
	// radio button groups
		...
		$name_val = ($name) ? $name : 'r-group-'.rand();
                ...
                $buttonGroupsContent = ($content) ? do_shortcode($content) : '';
                $output = <<<HTML
                        <ul class="wpfs button-group toggle {$domain} {$corners} {$custom_class}">
                                {$buttonGroupsContent}
                        </ul>
HTML;
                return $output;
...
HTML;
	...
	public static function button_option_groups($atts, $content = null){
		...
		$buttonGroupsContent = ($content) ? do_shortcode($content) : '';
                $output = <<<HTML
			<div class="wpfs button-group button-option-group {$domain} size-{$size}  {$custom_class}" data-grouptype="{$label}">
				{$buttonGroupsContent}
			</div>
HTML;
		return $output;
	...
	public static function button_option_group($atts, $content = null){
		...
		$output = <<<HTML
			<a href="{$url}" class="wpfs button {$class} {$corners} {$size}">{$buttonGroupContent}</a>
HTML;
		return $output;
	}
	...
	public static function social_login_button($atts, $content = null){
		...
		$output = <<<HTML
		<button href="#" class="wpfs {$type} button split {$domain} social-login-button {$corners} {$size} {$icon_side}-icon {$custom_class}"><span><i class="fa {$fa_class}"></i>
...
HTML;
	...
	public static function posts_cycle($atts, $content = null){
		...
		?>
                        <div class="wpfs slider posts-cycle slider-<?php echo $id;?> <?php echo $custom_class;?>">
...
HTML;
	...
	public static function posts_grid($atts, $content = null){
		...
		<ul class="wpfs small-block-grid-<?php echo $small_num;?> medium-block-grid-<?php echo $medium_num;?> large-block-grid-<?php echo $large_num;?> posts-grid <?php echo $custom_class;?>">
			<?php foreach ( $myposts as $post ) : setup_postdata( $post );?>
                        <li>
				<?php include(WP_Foundation_shortcodes_plugables::get_template_part('content', get_post_format() )); ?>
                        </li>
	...
	// posts_list
		...
		$myposts = get_posts( $args );
                ...
                ?>
		<div class="wpfs posts-list <?php echo $custom_class;?>">
			<?php foreach ( $myposts as $post ) : setup_postdata( $post );?>
	...
	// posts_lightbox
		...
		switch($thumbs){
			...
			break;
                ...
                ?>
		<ul class="wpfs clearing-thumbs <?php echo $custom_class;?> posts-lightbox" data-clearing>	
                        <?php foreach ( $myposts as $post ) : setup_postdata( $post );
	...
	// tags
		...
		$output = <<<HTML
			<div class="wpfs tags-cloud clearfix">{$tags_content}</div><!-- /.tags-cloud (end) -->
HTML;
                return $output;
	}
	...
	public static function categories($atts, $content = null){
		...
		switch($list_type){
			...
			case "square":
			case "no-bullet":
				$list_start = '<ul class="wpfs categories-list '.$list_type.' '.$custom_class.'">';
				$list_end = '</ul>';
			break;
			case "numbered":
				$list_start = '<ol class="wpfs categories-list '.$list_type.' '.$custom_class.'">';
                                $list_end = '</ol>';
			break;
			default:
...
HTML;
	...
	public static function comments($atts, $content = null){
		...
		foreach ($comments as $comment) {
                        $comments_content .= '<li class="list-item-'.$itemcounter.'">';
		...
		$output = <<<HTML
		<ul class="wpfs comments-list">{$comments_content}</ul>
HTML;
...
HTML;
                ...
                $output = <<<HTML
		<div class="wpfs banner-wraper {$custom_class} ">
			{$figure_val}
...
HTML;
		...
		$output = <<<HTML
		<div class="wpfs service-box panel radius {$custom_class}">
			{$figure_val}
